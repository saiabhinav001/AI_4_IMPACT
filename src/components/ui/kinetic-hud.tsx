    ...
    <div className="absolute inset-0 z-[5] pointer-events-none overflow-hidden">
      {HUD_ELEMENTS.map((el, i) => {
        ...
        return (
          ...
          >
            {el.type === "marker" && (
              <div className="h-4 w-4 border border-cyan-500/50 flex items-center justify-center">
                <div className="h-1 w-1 bg-cyan-400" />
              </div>
            )}
            {el.type === "bracket" && (
              <div className="text-[10px] font-mono text-[#8D36D5] whitespace-nowrap">
                [ 0x_LINK_0{i} ]
              </div>
            )}
            {el.type === "hex" && (
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[8px] font-mono text-zinc-600">OFFSET_0x{i*10}</span>
              </div>
            )}
          </motion.div>
        );
